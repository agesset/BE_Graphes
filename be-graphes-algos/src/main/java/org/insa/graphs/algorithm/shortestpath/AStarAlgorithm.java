...
public class AStarAlgorithm extends DijkstraAlgorithm {
    ...
    protected ShortestPathSolution doRun() {
        ...
        labels[data.getOrigin().getId()] = new LabelStar(data.getOrigin(), false, 0.0, null);
        ...
        while (!found && !heap.isEmpty()) {
            ...
            for (Arc successor : currentLabel.getCurrentNode().getSuccessors()) {
                ...
                double newCost = currentLabel.getTotalCost() + data.getCost(successor);
                LabelStar successorLabel = labels[successor.getDestination().getId()];

                if (successorLabel == null) {
                    successorLabel = new LabelStar(
        ...
        if (labels[data.getDestination().getId()] == null
            ...
            solution = new ShortestPathSolution(data, AbstractSolution.Status.OPTIMAL,
                    new Path(graph, arcs));
        }
